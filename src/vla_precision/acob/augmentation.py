...
from vla_precision.acob.types import Batch, PRNGKey
...
def make_batch_augmentation(image_keys) -> callable:
    """Return the exact per-camera random-crop augmentation used by ACoB."""

    def augment_observations(rng, observations):
        for image_key in image_keys:
            observations = observations.copy(
                add_or_replace={
                    image_key: batched_random_crop(
                        observations[image_key], rng, padding=4, num_batch_dims=2
                    )
                }
            )
        return observations
    ...
    def augment_batch(batch: Batch, rng: PRNGKey) -> Batch:
        ...
        observations = augment_observations(obs_rng, batch["observations"])
        next_observations = augment_observations(next_obs_rng, batch["next_observations"])
        return batch.copy(
            add_or_replace={
