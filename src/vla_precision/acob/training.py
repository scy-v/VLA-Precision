"""Batch, optimizer and JAX TrainState infrastructure for ACoB."""
...
from vla_precision.acob.types import Params, PRNGKey
...
def unpack_memory_efficient_batch(batch):
    """Split adjacent pixel frames stored together by the replay buffer."""
    for pixel_key in batch["observations"]:
        if pixel_key not in batch["next_observations"]:
            ...
            observations = batch["observations"].copy(add_or_replace={pixel_key: obs_pixels})
...
def make_optimizer(
    ...
    def optimizer(learning_rate: float, weight_decay: Optional[float]):
        ...
        if weight_decay is not None:
            optimizer_stages.append(optax.adamw(learning_rate=learning_rate, weight_decay=weight_decay))
        else:
