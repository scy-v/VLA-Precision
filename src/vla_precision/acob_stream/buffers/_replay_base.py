...
import jax
import numpy as np
from vla_precision.acob_stream.buffers._dataset import Dataset, DatasetDict
...
class ReplayBuffer(Dataset):
    def __init__(
        ...
        reward_shape = (action_space.shape[0],) if len(action_space.shape) >= 2 else ()
        dataset_dict = dict(
            observations=observation_data,
            next_observations=next_observation_data,
            actions=np.empty((capacity, *action_space.shape), dtype=action_space.dtype),
            rewards=np.empty((capacity, *reward_shape), dtype=np.float32),
        ...
        if include_context:
            dataset_dict['context_embeddings'] = np.empty((capacity, 384), dtype=np.float32)
            dataset_dict['next_context_embeddings'] = np.empty((capacity, 384), dtype=np.float32)
        ...
        if include_grasp_penalty:
            dataset_dict['grasp_penalty'] = np.empty((capacity, *reward_shape), dtype=np.float32)
